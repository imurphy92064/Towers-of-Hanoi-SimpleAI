...
void tracePath(state goal){
  ...
  //If the goal's state isn't RRRR then we display the state and recursively calls this function
  //With the cameFrom state of goal.
  if(goal.items != "111"){
    cout<<"Path Node: {";
    cout << "{ State: ";
...
void generate(string current, state next){
    /* if unsafe, do not add to frontier.
      ...
      If true next is an unsafe state*/
    ...
    /* if been there before, do not add to frontier.
      ...
      If false next has not been visited*/
    if(visited(next)){
